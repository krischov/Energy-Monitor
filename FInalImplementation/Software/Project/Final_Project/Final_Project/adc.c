...
void adc_init() {
	...
	ADMUX = 0b01000000;
	//Enables ADC in auto trigger mode, enables complete interupt
	ADCSRA = 0b11101010;
...
void adc_read_voltage(){
	for (uint8_t i = 0; i < 20; i++) {
		...
		v_vs[i] = ((float)(adc_val * 5) / (float) 1024 - 2.1) / (float) (0.0454016298);//GvsGvo
	}
}
...
void adc_read_current(){
	for (uint8_t i = 0; i < 20; i++) {
		...
		i_is[i] = (float) ((adc_val * 5) / (float) 1024 - 2.1) / (float) (1.146853147);//GisGio
	}
}
