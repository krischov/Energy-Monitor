...
void Disp_ScanNext(){
	...
	//1. Load the next digit position and the number to displayed
	// e.g. if value to display is 1234 and current position (DispPos) is 0
	// this would mean we last displayed the `ones` column, value 4
	// therefore
	// we should now load the `tens` column, which is value 3
	// (Remember that the position is stored as DispPos)
	//2. Call Disp_Send() with number to display and position to display it at
